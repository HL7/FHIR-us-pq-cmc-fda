...
//-------------------------Batch Analysis------------------------------------------------------------------------------------//
Extension: DrugSubstancemanufacturingBatch
//fhir.uv.extensions is a dependency so it can be referenced by its
//human readable name, not its uri
Parent: MedManufacturingBatch
...
* extension[manufacturingDate] 1..1 MS
  ...
  * ^definition = """The date associated with manufacturing a batch. [Source: SME Defined]* note: See Manufacturing Date Description element. """
  * valueDateTime MS
* extension[manufacturingDateClassification] 1..1 MS
  * ^short = "Manufacturing Date Description"
  * ^definition = """A textual description that provides a rationale for the selection of the manufacturing date. [Source: SME Defined]* note: This description should include the specific operation/step in the manufacturing process associated with the assigned manufacturing date. """
...
* extension[expirationDateClassification] 1..1 MS
  ...
  * ^definition = """The endorsement of the Retest date that clarifies whether this date has been approved by the FDA or is being proposed by the sponsor/applicant for a drug substance. [Source: SME Defined]
...
"""
  * valueCodeableConcept MS
  * valueCodeableConcept from PqcmcRetestDateClassificationTerminology (required)
* extension[batchUtilization] 1..1 MS
  * ^short = "Batch Utilization"
  * ^definition = """A categorization of the batch that identifies its usage. [Source: SME Defined] * Examples: commercial, development. """
  * valueCodeableConcept MS
  * valueCodeableConcept from PqcmcBatchUtilizationTerminology (required)
* extension[batchQuantity] 1..1 MS
  * ^short = "Batch Size"
  * ^definition = "The batch size can be defined either by a fixed quantity or by the amount produced in a fixed time interval. [Source: ICH Q7 - Part of the definition of Batch]"
  * valueQuantity MS
* extension[additionalInformation] MS
  * ^short = "Additional Information"
  * ^definition = """A placeholder for providing any comments that are relevant to the Batch. [Source: SME Defined]
...
* extension[container] 1..* MS
  ...
  * extension[type] 1..1 MS
    ...
    * ^definition = "The kind of container that drug substances and finished dosage forms are contained in, which could include both the immediate (or primary) and secondary containers [Source: Adapted from NCI Thesaurus C43164]"
    * valueCodeableConcept MS
  ...
  * extension[quantity] 1..1 MS
    ...
    * ^definition = """Container Fill: Amount or volume of the drug product in the container. [Source: SME Defined]. Examples: 100 tablets; 10 mL, 1 transdermal system, 1 sachet, etc. Note: the examples include both the Container Fill and the Container Fill Unit
