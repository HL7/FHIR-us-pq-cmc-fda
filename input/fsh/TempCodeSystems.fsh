...
// FMG doesn't like the dummy NCIt system, so it's getting removed. One ValueSet needs 
// a hierarchy to be defined so we're trying to do that with a supplement that uses
// properties

// This is what I would expect to work, but doesn't. ran with latest publisher (1.7.2),
// errors saying 'child' is already defined. Don't know how csd-2 or csd-3 could be
// satisfied with a hierarchical supplement, hierarchyMeaning can't be set

CodeSystem: NCItTestCategoryHierarchySupplement
Id: cmc-test-category-hiearchy-supplement
...
// all the others commented out for testing
